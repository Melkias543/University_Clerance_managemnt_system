...
import { CafeteriaApplication } from "../../models/applicationModel.js";
import applicationService from "../../services/applicationService.js";
const sendToCafteriaApplication = async (req, res) => {
  try {
    ...
    const sendedAppilicationData =
      await applicationService.sendAppilicationData(CafeteriaApplication, data);
...
const getSingleCafteria = async (req, res) => {
  try {
    const { id } = req.params;
    // console.log("here",id)
...
const aproveOrRejectAtCafteria = async (req, res) => {
  try {
    const { id } = req.params;
    const { status } = req.body;
const office = "Cafeteria";
    if (!mongoose.Types.ObjectId.isValid(id) || !id) {
      return res.status(400).json({
        status: false,
        msg: "Invalid service ID or ID is Not provided",
...
export default {
  sendToCafteriaApplication,
  getAllApplication,
  getSingleCafteria,
  aproveOrRejectAtCafteria,
};
