...
import { DepartmentHeadApplication } from "../../models/applicationModel.js";
import applicationService from "../../services/applicationService.js";
const sendToDeptHeadApplication = async (req, res) => {
  try {
...
const getSingleDepartmentHeadApplication = async (req, res) => {
  try {
    const { id } = req.params;
    // console.log("here",id)
...
const aproveOrRejectAtDeptHead = async (req, res) => {
  try {
    const { id } = req.params;
    const { status } = req.body;
const office = "departmentHead";
    if (!mongoose.Types.ObjectId.isValid(id) || !id) {
      return res.status(400).json({
        status: false,
        msg: "Invalid service ID or ID is Not provided",
...
export default {
  sendToDeptHeadApplication,
  getAllApplication,
  getSingleDepartmentHeadApplication,
  aproveOrRejectAtDeptHead,
};
