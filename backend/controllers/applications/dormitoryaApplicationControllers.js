...
const sendDormitoryApplication = async (req, res) => {
  try {
    ...
    // console.log("CheckSpecificAppliction", CheckSpecificAppliction);
    ...
    const sendedAppilicationData =
      await applicationService.sendAppilicationData(DormitoryApplication, data);
...
const getSingleDormitoryHeadApplication = async (req, res) => {
  try {
    const { id } = req.params;
    // console.log("here",id)
...
const aproveOrRejectAtDromitory = async (req, res) => {
  try {
    const { id } = req.params;
    const { status } = req.body;
const office = "Dormitory";
    if (!mongoose.Types.ObjectId.isValid(id) || !id) {
      return res.status(400).json({
        status: false,
        msg: "Invalid service ID or ID is Not provided",
...
const EditAplication = () => {
  try {
  } catch (error) {}
};

export default {
  sendDormitoryApplication,
  getSingleDormitoryHeadApplication,
  getAllApplication,
  getAplication,
  EditAplication,
  aproveOrRejectAtDromitory,
};
