...
import { LibraryApplication } from "../../models/applicationModel.js";
...
const sendLibraryApplication = async (req, res) => {
  try {
    ...
    const sendedAppilicationData =
      await applicationService.sendAppilicationData(LibraryApplication, data);
...
const getSingleLibraryApplication = async (req, res) => {
  try {
    const { id } = req.params;
    // console.log("here",id)
...
const aproveOrRejectAtLibrary = async (req, res) => {
  try {
    const { id } = req.params;
    const { status } = req.body;
const office = "Library";
    if (!mongoose.Types.ObjectId.isValid(id) || !id) {
      return res.status(400).json({
        status: false,
        msg: "Invalid service ID or ID is Not provided",
...
export default {
  sendLibraryApplication,
  getAllApplication,
  getSingleLibraryApplication,
  aproveOrRejectAtLibrary,
};
