...
import { RegistralApplication } from "../../models/applicationModel.js";
import applicationService from "../../services/applicationService.js";
const sendToRegistralApplication = async (req, res) => {
  try {
   ...
   const {
    ...
    const sendedAppilicationData =
      await applicationService.sendAppilicationData(RegistralApplication, data);
...
const getSingleRegistralApplication = async (req, res) => {
  try {
    const { id } = req.params;
    // console.log("here",id)
...
const aproveOrRejectAtRegistral = async (req, res) => {
  try {
    const { id } = req.params;
    const { status } = req.body;
const office = "Registral";
    if (!mongoose.Types.ObjectId.isValid(id) || !id) {
      return res.status(400).json({
        status: false,
        msg: "Invalid service ID or ID is Not provided",
...
export default {
  sendToRegistralApplication,
  getAllApplication,
  getSingleRegistralApplication,
  aproveOrRejectAtRegistral,
};
